...
export class PasswordGenerator {
  ...
  private static readonly LowercaseCharacters: string =
    "abcdefghijklmnopqrstuvwxyz";
  private static readonly UppercaseCharacters: string =
    "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  ...
  public Generate(generatePasswordDto: GeneratePasswordDto): string {
    ...
    if (generatePasswordDto.IncludeNumbers)
      charSet += PasswordGenerator.Numbers;

    if (generatePasswordDto.IncludeLowercaseCharacters)
      charSet += PasswordGenerator.LowercaseCharacters;

    if (generatePasswordDto.IncludeUppercaseCharacters)
      charSet += PasswordGenerator.UppercaseCharacters;

    if (generatePasswordDto.IncludeSpecialCharacters)
      charSet += PasswordGenerator.SpecialCharacters;

    if (
      !generatePasswordDto.IncludeNumbers &&
      !generatePasswordDto.IncludeLowercaseCharacters &&
      !generatePasswordDto.IncludeUppercaseCharacters &&
      !generatePasswordDto.IncludeSpecialCharacters
    )
      return "";
